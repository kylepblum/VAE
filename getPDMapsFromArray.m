...
for filenum = 1:numel(filename)
    ...
    %Get speed
    trial_data.speed = sqrt(trial_data.vel(:,1).^2 + trial_data.vel(:,2).^2);
...
hfig3 = figure; polarhistogram(S1_pds,72)
...
%% 
binnedPDs = zeros(size(pdTable,1),length(0:0.1:180));

edges = round(rad2deg(wrapTo2Pi(pdTable.velPDCI))*10)/10;
edgesIdx = edges * 10 + 1;
widths = diff(edges')';
heights = 1./widths;

for i = 1:size(binnedPDs,1)
    binnedPDs(i,edgesIdx(i,1):edgesIdx(i,2)) = heights(i);
end

plot(sum(binnedPDs)/size(binnedPDs,1))
